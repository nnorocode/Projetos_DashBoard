...
const statusOptions = ["Não iniciado", "Em andamento", "Concluído"];
...
const els = {
  ...
  viewActivitiesButton: document.querySelector("#viewActivitiesButton"),
...
async function saveActivities() {
  ...
  const { error } = await supabaseClient
    .from('atividades') // use 'atividades' se sua tabela estiver minúscula
...
async function syncFromCloud() {
  ...
  if (data && data.length > 0) {
    activities = data.filter(a => a.project === activeProject);
    projectActivities[activeProject] = activities;
    render(); 
  } else if (data && data.length === 0 && activities.length > 0) {
    const atividadesParaNuvem = activities.map(act => ({
      ...act,
      project: activeProject 
    }));
    const { error: errUp } = await supabaseClient.from('atividades').upsert(atividadesParaNuvem);
...
function getProjectPhases(project = activeProject) {
  ...
  return Array.isArray(selected) && selected.length ? selected.slice(0, 8) : phases.slice(0, 8);
}

function phaseCandidates() {
  return Array.from(new Set([...getProjectPhases(), ...phases])).slice(0, 12);
}
...
function setActiveProject(project) {
  ...
  if (!Array.isArray(projectPhases[project])) projectPhases[project] = phases.slice(0, 8);
...
function isLate(activity) {
  if (activity.status === "Concluído") return false;
...
function calcStats(items = activities) {
  ...
  const onTime = items.filter((item) => item.status !== "Concluído" && !isLate(item)).length;
  const todo = items.filter((item) => item.status === "Não iniciado").length;
  const risk = items.filter((item) => item.priority === "Alta" && item.status !== "Concluído").length;
...
function filteredActivities() {
  return activities.filter((item) => {
    ...
    const statusMatch =
      ...
      (activeStatus === "No prazo" && item.status !== "Concluído" && !isLate(item)) ||
      (activeStatus === "Prioridade alta" && item.priority === "Alta" && item.status !== "Concluído");
...
function renderTable() {
  ...
  els.table.innerHTML = rows
    .map((activity, index) => {
      ...
      return `
         <tr draggable="true" data-id="${activity.id}">
          <td>${index + 1}</td>
...
function renderRisks() {
  const risks = activities
    .filter((item) => item.blocker && item.status !== "Concluído")
...
function openPhaseModal() {
  const selected = new Set(getProjectPhases());
  els.phaseOptions.innerHTML = phaseCandidates()
    .map(
      (phase) => `<label class="phase-edit-row"><input name="projectPhase" type="checkbox" value="${phase}" data-original="${phase}" ${
        selected.has(phase) ? "checked" : ""
      } /><input class="phase-name-input" name="projectPhaseName" type="text" value="${phase}" data-original="${phase}" aria-label="Nome da fase ${phase}" /></label>`
    )
    .join("");
...
document.addEventListener("click", (event) => {
  ...
  if (deleteButton) {
    ...
    supabaseClient.from('atividades').delete().eq('id', id).then(({ error }) => {
      if (error) {
...
els.confirmDeleteProjectButton.addEventListener("click", () => {
...
els.projectForm.addEventListener("submit", (event) => {
  ...
  if (projects.includes(projectName) && projectName !== editingProjectName) {
    showToast("J existe um projeto com esse nome.");
    return;
  }
  if (editingProjectName) {
    projects = projects.map((project) => (project === editingProjectName ? projectName : project));
    projectActivities[projectName] = projectActivities[editingProjectName] || [];
    ...
    projectPhases[projectName] = phases.slice(0, 8);
  }
...
els.phaseForm.addEventListener("submit", (event) => {
  event.preventDefault();
  const selected = Array.from(els.phaseForm.querySelectorAll('input[name="projectPhase"]:checked')).map((input) => {
    const nameInput = els.phaseForm.querySelector(`input[name="projectPhaseName"][data-original="${CSS.escape(input.dataset.original)}"]`);
    return {
      original: input.dataset.original,
      name: cleanText(nameInput?.value || input.value).trim(),
    };
  });
  if (!selected.length) {
    showToast("Selecione pelo menos uma fase.");
    return;
  }
  if (selected.length > 8) {
    showToast("Escolha no máximo 8 fases.");
    return;
  }
  const selectedNames = selected.map((item) => item.name).filter(Boolean);
  const uniqueNames = Array.from(new Set(selectedNames));
  if (selectedNames.length !== uniqueNames.length || selectedNames.length !== selected.length) {
...
els.viewActivitiesButton.addEventListener("click", () => {
...
els.exportButton.addEventListener("click", async () => {
  const stats = calcStats();
  const text = [
    "Resumo do Cronograma de Publicação",
    `Total de atividades: ${stats.total}`,
    `Realizadas: ${stats.done}`,
    ...
    ...activities
      ...
      .sort((a, b) => new Date(a.due) - new Date(b.due))
      .slice(0, 5)
      .map((item) => `- ${item.title} (${item.phase}) até ${formatDate(item.due)}`),
  ].join("\n");
